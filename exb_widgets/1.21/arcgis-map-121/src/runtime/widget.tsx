...
import { checkIsLive, isFullscreenEnabled, getFullscreenElement, requestFullscreen, exitFullscreen, watchPopupElementPropertyChange } from './utils'
import {
  type JimuMapViewGroup, type ShowOnMapDatas, type AddToMapDatas, type JimuMapView, type JimuLayerView,
  DataChangeStatus, MapViewManager, type MarkerGroup, type UnparsedMapUrlParams
} from 'jimu-arcgis'
import { ViewportVisibilityContext } from 'jimu-layouts/layout-runtime'
import { versionManager } from '../version-manager'
import { Icon, DataActionList, DataActionListStyle, Paper } from 'jimu-ui'
...
export default class Widget extends React.PureComponent<MapWidgetProps, State> {
  ...
  mapLoadingBarContainer: HTMLElement
  readonly warningIcon: string
  readonly mapRootClassName: string
  watchUpdatingHandle: __esri.Handle
  watchStationaryHandle: __esri.Handle
  isRTL: boolean
  ...
  constructor (props) {
    ...
    this.dataActionListContainer.className = 'data-action-list-wrapper d-flex'
    this.dataActionListContainer.slot = 'actions-start'
  ...
  getWidgetStyle () {
    const localeRight = this.isRTL ? 'left' : 'right'
    ...
    return css`
      ...
      */
      ...
      .map-loading-indicator-container {
        position: absolute;
        ${localeRight}: 4px;
        bottom: 28px;
        pointer-events: none;
      ...
      &.is-mobile-layout {
        ...
        &.has-select-tool .map-loading-indicator-container {
          bottom: 44px;
        }
      }
  ...
  componentWillUnmount () {
    ...
    if (this.watchUpdatingHandle) {
      this.watchUpdatingHandle.remove()
      this.watchUpdatingHandle = null
    }

    if (this.watchStationaryHandle) {
      this.watchStationaryHandle.remove()
      this.watchStationaryHandle = null
    }
  }
  ...
  onActiveJimuMapViewChange = (jimuMapView: JimuMapView) => {
    ...
    this.updateDataActionDom()
    this.watchViewUpdating()
    this.updateMapLoadingBar()
  }
  ...
  getPopupElement (): HTMLArcgisPopupElement {
    return this.activeJimuMapView?.mapComponent?.popupElement
  }

  getCurrentPopupSelectedFeature (): __esri.Graphic {
    const popupElement = this.getPopupElement()
    return popupElement?.selectedFeature as unknown as __esri.Graphic
  }
  ...
  watchPopupSelectedFeatureChange () {
    ...
    this.watchSelectedFeatureHandle = watchPopupElementPropertyChange(popupElement, 'selectedFeature', () => {
      this.handleDataActionWhenPopupSelectedFeatureChange(activeJimuMapView)
    })
  }
  ...
  async handleDataActionWhenPopupSelectedFeatureChange (activeJimuMapView: JimuMapView) {
    const popupElement = this.getPopupElement()
    const originalSelectedFeature = popupElement?.selectedFeature as unknown as __esri.Graphic
  ...
  updateDataActionDom () {
    ...
    if (this.state.dataActionDataSet) {
      // this.state.dataActionDataSet is not empty, so we need to make sure this.dataActionListContainer is in popup
      if (this.dataActionListContainer.parentNode) {
        if (this.dataActionListContainer.parentNode !== popupElement) {
          this.dataActionListContainer.parentNode.removeChild(this.dataActionListContainer)
          popupElement.appendChild(this.dataActionListContainer)
        }
      } else {
        popupElement.appendChild(this.dataActionListContainer)
      }
    } else {
      // remove dataActionListContainer if this.state.dataActionDataSet is empty
      if (this.dataActionListContainer.parentNode) {
        this.dataActionListContainer.parentNode.removeChild(this.dataActionListContainer)
      }
    }
  ...
  async getDataActionDataSet (jimuMapView: JimuMapView, selectedFeature: __esri.Graphic) {
    let layerDataSource: DataSource
  ...
  async watchViewUpdating () {
    ...
    this.watchUpdatingHandle = jimuMapView.addUpdatingListener(this.onViewUpdating)

    this.watchStationaryHandle = await jimuMapView.watch(() => jimuMapView?.view?.stationary, () => {
      this.updateMapLoadingBar()
    })
  ...
  onViewUpdating = (targetJimuMapView, updating) => {
    const jimuMapView = this.activeJimuMapView
    if (jimuMapView && jimuMapView === targetJimuMapView && jimuMapView?.isActive) {
      this.updateMapLoadingBar()
    }
  }

  updateMapLoadingBar () {
    if (!this.mapLoadingBarContainer) {
      return
    }

    const jimuMapView = this.activeJimuMapView
    const showLoading = jimuMapView && jimuMapView?.view?.stationary && jimuMapView.isUpdating()

    if (showLoading) {
      this.mapLoadingBarContainer.classList.remove('d-none')
    } else {
      this.mapLoadingBarContainer.classList.add('d-none')
    }
  }

  onMapLoadingBarContainerRef = (ref) => {
    this.mapLoadingBarContainer = ref
    this.updateMapLoadingBar()
  }

  render () {
    const dataActionList = this.renderDataActionList()
    const isMobileLayout = this.state.widthBreakpoint === 'xsmall'
    const hasSelectTool = this.props.config.toolConfig?.canSelect
    const className = classNames(`jimu-widget arcgis-map ${this.mapRootClassName}`, {
      ...
      'has-select-tool': hasSelectTool
    })

    return (
      <Paper shape='none' className={className} css={this.getWidgetStyle()}>
        <Global styles={this.getWidgetGlobalStyle()} />
        <div className='map-loading-indicator-container' ref={this.onMapLoadingBarContainerRef}>
          <DotLoading size={ isMobileLayout ? 5 : 6} />
        </div>

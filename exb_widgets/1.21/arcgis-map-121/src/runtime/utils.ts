...
import {
  ...
  type MarkerGroupUrlParam, type MarkerPointUrlParam, type MarkerGroup, type UnparsedMapUrlParams, type UnparsedViewUrlParams, type JimuQueriableLayerView,
} from 'jimu-arcgis'
...
type SupportQueryExtentLayerType = __esri.FeatureLayer | __esri.SceneLayer | __esri.BuildingComponentSublayer | __esri.SubtypeGroupLayer | __esri.KnowledgeGraphSublayer
...
export function layerQueryExtent (
  layer: SupportQueryExtentLayerType | __esri.SubtypeSublayer,
  query: __esri.Query
  ...
  if (layer.declaredClass === 'esri.layers.support.SubtypeSublayer') {
    ...
    return (layer as SupportQueryExtentLayerType).queryExtent(query)
  }
}
...
export async function filterFeaturesByQuery (jimuMapView: JimuMapView, actionLayerDataSourceId: string, querySQL: string): Promise<void> {
  ...
  let jimuLayerView: JimuQueriableLayerView

  try {
    jimuLayerView = await jimuMapView.whenJimuLayerViewLoadedByDataSource(dataSource) as JimuQueriableLayerView
  } catch (err) {
    jimuLayerView = null
...
type SupportQueryFeaturesLayerTypes = __esri.FeatureLayer | __esri.ImageryLayer | __esri.OrientedImageryLayer | __esri.SubtypeGroupLayer | __esri.SubtypeSublayer | __esri.KnowledgeGraphSublayer
...
export async function flashFeaturesByQuery (jimuMapView: JimuMapView, layerDataSourceId: string, querySQL: string): Promise<void> {
  ...
  let jimuLayerView: JimuLayerView
  ...
  if (jimuLayerView && jimuLayerView.layer) {
    ...
    const supported2DLayers: JSAPILayerTypes[] = [
      JSAPILayerTypes.FeatureLayer,
      JSAPILayerTypes.ImageryLayer,
      JSAPILayerTypes.OrientedImageryLayer,
      JSAPILayerTypes.SubtypeGroupLayer,
    ...
    const supported3DLayers: JSAPILayerTypes[] = [
      JSAPILayerTypes.SceneLayer,
      JSAPILayerTypes.BuildingComponentSublayer
    ]
...
export function updateAriaRoleForViewSurface (view: __esri.MapView | __esri.SceneView, ariaRole: string) {
  if (!view) {
    return
  }

  if (!ariaRole) {
    // application is the default role of map
  ...
  if (surfaceDom) {
    surfaceDom.setAttribute('role', ariaRole)
  }
}

function addEventListener (element: HTMLElement, eventName: string, listener: (event: CustomEvent) => void): __esri.Handle {
...
export function watchPopupElementPropertyChange (popupElement: HTMLArcgisPopupElement, propertyName: string | null, listener: (event: CustomEvent) => void, delayMillSeconds: number = 50): __esri.Handle {
  const isValidDelay = typeof delayMillSeconds === 'number' && delayMillSeconds >= 0

  if (!isValidDelay) {
    delayMillSeconds = 0
  }

  let openChangeTimer: ReturnType<typeof setTimeout> | null = null

  const finalListener = (event: CustomEvent) => {
    const eventPropertyName = event?.detail?.name

    if (propertyName) {
      if (eventPropertyName !== propertyName) {
        return
      }
    }
    ...
    // The popup element triggers the arcgisPropertyChange event twice in succession for the same property:
    // When a feature is selected by clicking, the selectedFeature event is triggered twice consecutively with a 1~3ms interval.
    // When opening the Popup, the open event is triggered twice in succession with a 0ms interval.
    // When closing the Popup, the open event is triggered twice in succession with a 1ms interval.
    // To fix this strange API behavior, we need to use setTimeout; to be safe, we are setting the timeout to delayMillSeconds, default is 50ms.
    openChangeTimer = setTimeout(() => {
      openChangeTimer = null
      listener(event)
    }, delayMillSeconds)
  }

  return addEventListener(popupElement, 'arcgisPropertyChange', finalListener)
}

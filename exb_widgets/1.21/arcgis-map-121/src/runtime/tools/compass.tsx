...
class CompassInner extends React.PureComponent<CompassInnerProps, any> {
  compassElement: HTMLArcgisCompassElement
  ...
  tryUpdateCompassElement () {
    ...
    // destroy current compassElement when map component changed or container changed
    if (this.compassElement) {
      if (this.compassElement.referenceElement !== currMapComponent || this.compassElement.parentNode !== this.container) {
        this.destroyCompassElement()
      }
    }

    // create new compassElement if this.compassElement is empty and this.container & currMapComponent are not empty
    if (!this.compassElement && this.props.mapComponentsLoaded && this.container && currMapComponent) {
      this.compassElement = document.createElement('arcgis-compass')
      this.compassElement.referenceElement = currMapComponent
  ...
  destroyCompassElement () {
    if (this.compassElement) {
      const view = (this.compassElement.referenceElement as HTMLArcgisMapElement | HTMLArcgisSceneElement)?.view

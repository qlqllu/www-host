...
class SearchInner extends React.PureComponent<SearchInnerProps, SearchInnerState> {
  ...
  tryUpdateSearchElement () {
    ...
    // destroy current searchElement when map component or container changed
    if (this.searchElement) {
      if (this.searchElement.referenceElement !== currMapComponent || this.searchElement.parentNode !== this.container) {
        this.destroySearchElement()
      }
    }

    // create new searchElement if this.searchElement is empty and this.container & currMapComponent are not empty
    if (!this.searchElement && this.props.mapComponentsLoaded && this.state.apiLoaded && this.container && currMapComponent) {
      this.searchElement = document.createElement('arcgis-search')
      this.searchElement.referenceElement = currMapComponent
  ...
  updateSearchPopupEnabled () {
    if (this.searchElement) {
      const view = (this.searchElement.referenceElement as HTMLArcgisMapElement | HTMLArcgisSceneElement)?.view

      if (view) {
        this.searchElement.popupDisabled = !view.popupEnabled
      }
    }
  }

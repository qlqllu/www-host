...
import {
  ...
  css, jsx, DataSourceManager, getAppStore, polished, classNames, type UseDataSource, AllDataSourceTypes, type ImmutableArray, loadArcGISMapComponents
} from 'jimu-core'
...
import { type ExBPopupDockPosition, type CustomPopupDockPosition, getValidPopupDockPosition, getMinScaleAndMaxScaleByLods, isExpressMode } from '../utils'
...
interface ScaleRangeSliderPropertyChangeDetail {
  name?: 'minScale' | 'maxScale'
}

const validScaleRangeSliderRegions: Array<HTMLArcgisSliderScaleRangeElement['region']> = [
...
function getScaleRangeSliderRegion (locale?: string): HTMLArcgisSliderScaleRangeElement['region'] {
  const region = locale?.split('-')?.pop()?.toUpperCase() as any

  if (region && validScaleRangeSliderRegions.includes(region)) {
    return region
  }
...
export default class Setting extends React.PureComponent<MapSettingProps, State> {
      ...
      .exb-scale-range-slider-setting-row {
        .exb-scale-range-slider-container {
          ...
          arcgis-slider-scale-range {
            ...
            --calcite-button-text-color: var(--sys-color-surface-overlay-text, #FFFFFF);
            --calcite-dropdown-item-background-color-hover: var(--sys-color-action-hover, #1A1A1A);
            --arcgis-scale-range-slider-segment-color: var(--sys-color-action-selected, #007F94);
            --calcite-color-text-3: var(--sys-color-action-selected, #007F94);
            --calcite-list-description-text-color: #dcdcdc;
            --calcite-list-background-color: #282828;
          }
        }
      }
    `
  ...
  componentDidMount () {
    ...
    this.loadScaleRangeSliderComponent()
  }
  ...
  async updateScaleRangeSlider () {
    ...
    const mapComponent = this.activeJimuMapView?.mapComponent
    const apiMapView = this.activeJimuMapView?.view
    const config = this.props.config

    if (this.mapComponentsLoaded && mapComponent && apiMapView && this.scaleRangeSliderContainer && config?.scaleRange) {
      const scaleRangeSlider = document.createElement('arcgis-slider-scale-range')
      scaleRangeSlider.referenceElement = mapComponent

      if (typeof this.minScale === 'number') {
        scaleRangeSlider.minScale = this.minScale
      }

      if (typeof this.maxScale === 'number') {
        scaleRangeSlider.maxScale = this.maxScale
      }

      const locale = getAppStore().getState()?.appContext?.locale
      const region = getScaleRangeSliderRegion(locale)

      if (region) {
        scaleRangeSlider.region = region
      }

      this.addScaleRangeSliderListeners(scaleRangeSlider)
      this.scaleRangeSliderContainer.replaceChildren(scaleRangeSlider)
      this.scaleRangeSlider = scaleRangeSlider

      this.scaleHandleOfMapView = await this.activeJimuMapView.watch(() => apiMapView.scale, () => {
        if (this.scaleRangeSlider) {
          // Resetting the referenceElement keeps the current scale indicator aligned with the active map view.
          this.scaleRangeSlider.referenceElement = undefined
          this.scaleRangeSlider.referenceElement = mapComponent
        }
      })
  ...
  addScaleRangeSliderListeners (scaleRangeSlider: HTMLArcgisSliderScaleRangeElement) {
    if (scaleRangeSlider) {
      scaleRangeSlider.addEventListener('arcgisPropertyChange', this.onScaleRangeSliderPropertyChange as EventListener)

      scaleRangeSlider.componentOnReady?.().then(() => {
        if (this.scaleRangeSlider === scaleRangeSlider) {
          this.updateScaleRangeLimitState(scaleRangeSlider)
        }
      }).catch((err) => {
        console.error('scaleRangeSlider componentOnReady error', err)
      })
    }
  }

  removeScaleRangeSliderListeners (scaleRangeSlider: HTMLArcgisSliderScaleRangeElement) {
    if (scaleRangeSlider) {
      scaleRangeSlider.removeEventListener('arcgisPropertyChange', this.onScaleRangeSliderPropertyChange as EventListener)
    }
  }

  onScaleRangeSliderPropertyChange = (evt: CustomEvent<ScaleRangeSliderPropertyChangeDetail>) => {
    const scaleRangeSlider = evt.currentTarget as HTMLArcgisSliderScaleRangeElement
    const propertyName = evt?.detail?.name

    if (!scaleRangeSlider || this.scaleRangeSlider !== scaleRangeSlider) {
      return
    }

    if (propertyName === 'minScale') {
      this.minScale = scaleRangeSlider.minScale
      this.debounceHandleMinMaxScaleChange()
    } else if (propertyName === 'maxScale') {
      this.maxScale = scaleRangeSlider.maxScale
      this.debounceHandleMinMaxScaleChange()
    }
  }

  updateScaleRangeLimitState (scaleRangeSlider: HTMLArcgisSliderScaleRangeElement) {
  ...
  removeScaleRangeSlider () {
    ...
    if (this.scaleRangeSlider) {
      ...
      this.removeScaleRangeSliderListeners(scaleRangeSlider)
      scaleRangeSlider.remove()
      this.scaleRangeSlider = null
      void scaleRangeSlider.destroy?.().catch((err) => {
        console.error('destroy scale range slider component error', err)
      })

      if (this.scaleRangeSliderContainer) {
        this.scaleRangeSliderContainer.replaceChildren()
      }
    }
  ...
  resetMinScaleAndMaxScale () {
    ...
    if (this.scaleRangeSlider) {
      this.removeScaleRangeSliderListeners(this.scaleRangeSlider)
      this.scaleRangeSlider.minScale = 0
      this.scaleRangeSlider.maxScale = 0
      this.addScaleRangeSliderListeners(this.scaleRangeSlider)
    }
  }
  ...
  onPopupDockPositionChange = (newPosition: ExBPopupDockPosition) => {
    const config = this.props.config
  ...
  getNewConfigByEnableDataSourceClientQuery = (oldConfig: IMConfig, dataSourceId: string): IMConfig => {
    ...
    let newClientQueryDataSourceIds: ImmutableArray<string>
  ...
  render () {
    ...
    const popupDockPosition: ExBPopupDockPosition = enablePopUp ? getValidPopupDockPosition(this.props.config) : null
    ...
    const clientQueryHelpTip = this.props.intl.formatMessage({

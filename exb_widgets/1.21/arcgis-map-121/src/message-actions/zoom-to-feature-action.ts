...
import {
  filterMessageDescription, checkCycleForNewMessageAction, validateRecordDataSourceForRecordsSelectionChangeMessage,
  getValidChangedDataSourceIdsAndAllUseDataSourceIdsForDataFilteringChangeMessage, getInitialMessages, getOutputMessages
...
export default class ZoomToFeatureAction extends AbstractMessageAction {
  ...
  filterMessageDescription (messageDescription: MessageDescription): boolean {
    return filterMessageDescription(messageDescription) && !checkCycleForNewMessageAction(messageDescription, this)
  }
  ...
  getDefaultMessageActionConfig (message: Message): ZoomToMessageConfig {
    ...
    // If message.dataSourceId exists, it indicates that this message action is being configured on the data source panel.
    // Under this circumstance, configuring "all data" as the trigger is not allowed.
    // Therefore, message.dataSourceId needs to be checked here.
    if (!message.dataSourceId && (message.type === MessageType.DataRecordsSelectionChange || message.type === MessageType.DataSourceFilterChange)) {
  ...
  onExecute (message: Message, actionConfig?: IMConfig): Promise<boolean> | boolean {
    return loadArcGISJSAPIModules(['esri/Graphic']).then(modules => {
      ...
      let zoomToOption: ZoomToOptions
        ...
        case MessageType.DataRecordSetChange:
          ...
          if (dataRecordSetChangeMessage.changeType === RecordSetChangeType.CreateUpdate) {
            ...
            if (dataRecordSetChangeMessage.extent) {
              ...
              // When the Search widget searches across multiple sources:
              // 1. It first triggers a DataRecordsSelectionChangeMessage, then we update zoomToFeatureActionValue via MutableStoreManager.
              //    Note that the handleActionForZoomToFeatureActionValue method is not executed immediately.
              // 2. Shortly after (70–500ms), a DataRecordSetChange message is sent. In this case, arrayFeatures might be an empty array.
              //    If we update zoomToFeatureActionValue using this empty array, it will overwrite and delete the value from Step 1, resulting in no zoom at all.
              // To resolve this, we should only update zoomToFeatureActionValue in Step 1 and skip the update in Step 2 if arrayFeatures is empty.
              // This ensures the view correctly zooms to the selection. Therefore, we need to check if arrayFeatures is empty here.
  ...
  getInitialMessages (): Message[] {
    return getInitialMessages(this)
  }

  getOutputMessages (message: Message, actionConfig?: IMConfig): Message[] {
    return getOutputMessages(this, message, actionConfig)
  }
}

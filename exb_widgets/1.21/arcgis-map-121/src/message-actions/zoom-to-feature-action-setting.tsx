...
import {
  type ActionConfig, checkOutActionConfigForZoomToAndPanToMessageActions, getDataSourceSelectorSourceDataForZoomToAndPanToMessageActions,
  getDsByWidgetId
} from './action-utils'
...
const DSSelectorTypes = Immutable([
  ...
  AllDataSourceTypes.KnowledgeGraphSublayer
])
...
class _ZoomToFeatureActionSetting extends React.PureComponent<ActionSettingProps<IMConfig> & ExtraProps, States> {
  ...
  componentDidMount () {
    ...
    const initConfig = checkOutActionConfigForZoomToAndPanToMessageActions(this.props.config, this.props.messageWidgetId, this.props.dataSourceId, this.props.messageType)
  ...
  handleTriggerLayerSelected = (currentSelectedDs: UseDataSource[]) => {
    let finalSelectedDs: UseDataSource[] | ImmutableArray<UseDataSource>
  ...
  getTriggerLayerContent = () => {
    ...
    const triggerDsSelectorSourceData = getDataSourceSelectorSourceDataForZoomToAndPanToMessageActions(
      this.props.messageWidgetId,
      this.props.dataSourceId,
      this.props.config.useDataSource,
      this.props.config.useDataSources,
      this.props.messageType
    )

    let useDataSources: ImmutableArray<UseDataSource>
    ...
    let hideDataViewProp: boolean | ((dataViewJson: IMDataViewJson, mainDataSourceId: string) => boolean)
    ...
    let isDisableDataSourceListProp: boolean
    ...
    if (this.props.dataSourceId) {
      isDisableDataSourceListProp = true
    }

    this.fromRootDsIds = triggerDsSelectorSourceData.fromRootDsIds
    const sidePopperPosition = this.props.dataSourceId ? 'left' : 'right'
      ...
      <div>
        ...
        <SettingRow>
          <DataSourceSelector
            ...
            sidePopperPosition={sidePopperPosition}
          />
        </SettingRow>
      </div>
    )

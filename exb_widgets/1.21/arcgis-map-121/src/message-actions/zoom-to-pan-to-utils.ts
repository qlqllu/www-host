import {
  ...
  type UseDataSource, Immutable, type ImmutableArray, type IMAppConfig, type IMMessageJson, type MessageActionJson,
  type MessageAction, type Message, ExtentChangeMessage, moduleLoader
} from 'jimu-core'
...
import { isExpressMode, getDsByWidgetId, isSpecialCaseOfSearchWidget, getAppConfig } from './action-utils'
...
export function getInitialMessages (actionInstance: MessageAction): Message[] {
  const extentMessage = new ExtentChangeMessage(actionInstance.widgetId, null, null, true)
  extentMessage.addRelatedWidgetId(actionInstance.widgetId)
  return [extentMessage]
}

export function getOutputMessages (actionInstance: MessageAction, message: Message, actionConfig?: ZoomToPanToIMConfig): Message[] {
  const newMessages: Message[] = []

  if (message.type === MessageType.ExtentChange) {
    const inputMessage = message as ExtentChangeMessage
    const relatedWidgetIds = inputMessage.getRelatedWidgetIds()

    if (relatedWidgetIds.includes(actionInstance.widgetId)) {
      return newMessages
    } else {
      const extentMessage = new ExtentChangeMessage(actionInstance.widgetId, null, null, true)
      extentMessage.setRelatedWidgetIds(relatedWidgetIds)
      extentMessage.addRelatedWidgetId(actionInstance.widgetId)
      newMessages.push(extentMessage)
    }
  } else {
    const extentMessage = new ExtentChangeMessage(actionInstance.widgetId, null, null, true)
    extentMessage.addRelatedWidgetId(actionInstance.widgetId)
...
export function checkCycleForNewMessageAction (triggerMessageDescription: MessageDescription, actionInstance: MessageAction): boolean {
  const messageType = triggerMessageDescription.messageType
  const triggerWidgetId = triggerMessageDescription.widgetId
  const triggerDataSourceId = triggerMessageDescription.dataSourceId
  const actionWidgetId = actionInstance.widgetId

  if (triggerWidgetId === actionWidgetId) {
    return false
  }

  const messageActionCycleUtils = moduleLoader.getModuleSync('jimu-for-builder')?.messageActionCycleUtils

  if (!messageActionCycleUtils) {
    return true
  }

  const appConfig = getAppConfig()
  const messageJson = getOrCreateMessageJson(appConfig, triggerMessageDescription)
  const randomId = Math.random().toString().slice(2)
  const actionName = actionInstance.name
  const newActionId = `${actionInstance.id}-${randomId}`
  ...
  if (messageType === MessageType.DataRecordsSelectionChange || messageType === MessageType.DataSourceFilterChange) {
    if (triggerDataSourceId) {
      const useDataSource = getUseDataSourceByDataSourceId(triggerDataSourceId)
  ...
  const newMessageActionJson: MessageActionJson = {
    actionId: newActionId,
    widgetId: actionInstance.widgetId,
    dataSourceId: triggerDataSourceId,
    sectionId: null,
    messageWidgetId: triggerWidgetId,
    actionName,
    config,
    description: '',
    version: ''
  ...
  const newMessageJson = messageJson.updateIn(['actions'], (actions) => actions.concat(newMessageActionJson))
  const newAppConfig = appConfig.setIn(['messageConfigs', newMessageJson.id], newMessageJson)
  const hasCycle = messageActionCycleUtils.checkCycle(newAppConfig, newMessageJson.id, newActionId)

  return hasCycle
}

function getOrCreateMessageJson (appConfig: IMAppConfig, messageDescription: MessageDescription): IMMessageJson {
  const messageType = messageDescription.messageType
  const triggerWidgetId = messageDescription.widgetId
  const triggerDataSourceId = messageDescription.dataSourceId
  const messageConfigs = appConfig.messageConfigs
  let resultMessageConfig: IMMessageJson

  if (messageConfigs) {
    const messageConfigKeys = Object.keys(messageConfigs)

    for (let i = 0; i < messageConfigKeys.length; i++) {
      const messageConfig = messageConfigs[messageConfigKeys[i]]
      if (messageConfig.messageType === messageType) {
        if ((messageConfig.widgetId && messageConfig.widgetId === triggerWidgetId) || (messageConfig.dataSourceId && messageConfig.dataSourceId === triggerDataSourceId)) {
          resultMessageConfig = messageConfig
          break
        }
      }
    }
  ...
  if (resultMessageConfig) {
    if (!resultMessageConfig.actions) {
      resultMessageConfig = resultMessageConfig.set('actions', [])
    }
  }

  if (!resultMessageConfig) {
    const id = 'messageConfig_' + Math.random().toString().slice(2)

    resultMessageConfig = Immutable({
      id,
      messageType,
      dataSourceId: triggerDataSourceId,
      widgetId: triggerWidgetId,
      actions: []
    })
  }

  return resultMessageConfig
}
...
export function filterMessageDescription (messageDescription: MessageDescription): boolean {
  ...
  if (messageDescription.messageType === MessageType.ExtentChange) {
    ...
    let messageWidgetUseDataSources: ImmutableArray<UseDataSource>

    if (messageDescription.widgetId) {
      messageWidgetUseDataSources = getDsByWidgetId(messageDescription.widgetId, messageDescription.messageType)
    } else if (messageDescription.dataSourceId) {
      const useDataSource = getUseDataSourceByDataSourceId(messageDescription.dataSourceId)

      if (useDataSource) {
        messageWidgetUseDataSources = Immutable([useDataSource])
      } else {
        messageWidgetUseDataSources = Immutable([])
      }
    }
...
function getUseDataSourceByDataSourceId (dataSourceId: string): UseDataSource {
  let useDataSource: UseDataSource
  const dataSourceManager = DataSourceManager.getInstance()
  const ds = dataSourceManager.getDataSource(dataSourceId)
...
export function getValidChangedDataSourceIdsAndAllUseDataSourceIdsForDataFilteringChangeMessage (
  filterChangeMessage: DataSourceFilterChangeMessage,
  actionConfig?: ZoomToPanToIMConfig
): ChangedDataSourceIdsAndAllUseDataSourceIds {
  ...
  if (_isUseAnyTriggerData) {
    ...
    // need to get the final valid data source ids by checking filterChangeMessage.dataSourceIds and useDataSourceIds
    validChangedDataSourceIds = useDataSourceIds.filter(dataSourceId => {
      if (dataSourceIdsOfMessage.includes(dataSourceId)) {
        return true
      } else {
        // Consider the following scenario:
        // The Filter widget is configured with two data sources: default view and custom view1. A 'Data filtering changes -> Map zoom to' message action has been configured only for custom view1.
        // When the Filter widget enables a filter on the default view, the Map receives the filtering changes message from the default view.
        // Under this circumstance, although no filtering changes message is directly received from custom view1, since custom view1 automatically inherits from the default view,
        // we should also consider the filtering change for custom view1 to be triggered. In this case, we should also execute the 'zoom to' action for custom view1.
        const ds = dsManager.getDataSource(dataSourceId)
        const mainDs = ds?.getMainDataSource()
        const mainDsId = mainDs?.id

        if (mainDsId && dataSourceIdsOfMessage.includes(mainDsId)) {
          return true
        }
    ...
    validChangedDataSourceIds = Array.from(new Set(validChangedDataSourceIds))
  }

  // validChangedDataSourceIds should not include tables
  validChangedDataSourceIds = validChangedDataSourceIds.filter(dataSourceId => {
    const ds = dsManager.getDataSource(dataSourceId)
    const hasGeometry = ds && ds.getGeometryType()
    return !!hasGeometry
  })

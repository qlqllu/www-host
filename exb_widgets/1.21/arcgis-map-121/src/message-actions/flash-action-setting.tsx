...
class _FlashActionSetting extends React.PureComponent<ActionSettingProps<IMConfig> & ExtraProps, States> {
  ...
  handleTriggerLayerChange = (useDataSources: UseDataSource[]) => {
    if (this.props.dataSourceId) {
      // When the message is published by data source, the trigger data source is determined, user cannot change it, so just return directly without doing anything.
      return
    }
  ...
  render () {
    ...
    const triggerDsSelectorSourceData = actionUtils.getDataSourceSelectorSourceDataForFilterAndFlashMessageActions(this.props.messageWidgetId, this.props.dataSourceId, this.props.messageType, this.props.config.messageUseDataSource)
    ...
    const actionDsSelectorSourceData = actionUtils.getDataSourceSelectorSourceDataForFilterAndFlashMessageActions(this.props.widgetId, null, this.props.messageType, this.props.config.actionUseDataSource)

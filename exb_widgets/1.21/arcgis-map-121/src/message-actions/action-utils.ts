 ...
 * useDataSources is the multiple selected trigger data, it is used if it supports multiple data sources.
 *
 * If actionConfig.useDataSource is null, it is the the first time to open the message action setting.
 * If actionConfig.useDataSource is not null, it is not the the first time to open the message action setting.
 *
 * @param actionConfig
 * @param messageWidgetId
 * @param bgDataSourceId
 * @param messageType
 * @returns
 */
export function checkOutActionConfigForZoomToAndPanToMessageActions (
  actionConfig: Immutable.ImmutableObject<ActionConfig>,
  messageWidgetId: string,
  bgDataSourceId: string,
  messageType: MessageType
): { useDataSource: IMUseDataSource, useDataSources: IMUseDataSource[] } {
  let useDataSource: IMUseDataSource = null
  const useDataSources: IMUseDataSource[] = []

  if (bgDataSourceId) {
    // background data source is the message trigger

    if (!actionConfig.useDataSource) {
      // If actionConfig.useDataSource is null, it is the the first time to open the message action setting.
      const bgUseDataSource = getUseDataSourceByDataSourceId(bgDataSourceId)

      if (bgUseDataSource) {
        const bgImUseDataSource = Immutable(bgUseDataSource)
        useDataSource = bgImUseDataSource
        useDataSources.push(bgImUseDataSource)
      }
    } else {
      // If actionConfig.useDataSource is not null, it is not the the first time to open the message action setting.
      // keep the original useDataSource and useDataSources in actionConfig
      useDataSource = actionConfig.useDataSource

      if (actionConfig.useDataSources?.length > 0) {
        actionConfig.useDataSources?.forEach(useDS => {
          useDataSources.push(useDS)
        })
      }
    }
  } else {
    // widget intersection is the message trigger
      ...
      if (messageWidgetUseDataSources.length > 0) {
        messageWidgetUseDataSources.forEach((useDS, index) => {
          if (!isWebMapOrWebSceneDataSourceId(useDS.dataSourceId)) {
            const tempUseDataSource = Immutable({
...
export function validateUseDataSource (widgetId: string, oldUseDataSource: IMUseDataSource, messageType: MessageType, compareMainDataSourceId: boolean): IMUseDataSource {
  let initUseDataSource: IMUseDataSource
 ...
 * @param bgDataSourceId bgDataSourceId is the data source id that publishes message in background
 * @param configUseDataSource
 * @param configUseDataSources
 * @param messageType
 * @returns
 */
export function getDataSourceSelectorSourceDataForZoomToAndPanToMessageActions (
  messageWidgetId: string,
  bgDataSourceId: string,
  configUseDataSource: ImmutableObject<UseDataSource>,
  configUseDataSources: ImmutableArray<UseDataSource>,
  messageType: MessageType
): DataSourceSelectorSourceDataForZoomToAndPanTo {
  let dsSelectorSource: DataSourceSelectorSourceDataForZoomToAndPanTo

  if (bgDataSourceId) {
    // case1: background data source is the message trigger
    // isReadOnly is true when there is only one layer main data source available as the trigger data candidate of message action.
    const isReadOnly = true
    const finalUseDataSource = configUseDataSource
    const finalUseDataSources = configUseDataSources || Immutable([])
    // Note, if rootIds is empty, we still need to return undefined (instead of empty array []) for code compatibility.
    const fromRootDsIds = undefined
    const fromDsIds = Immutable([bgDataSourceId])
    dsSelectorSource = {
      isReadOnly,
      useDataSource: finalUseDataSource,
      useDataSources: finalUseDataSources,
      fromRootDsIds,
      fromDsIds
    }
  } else {
    // case2: widget intersection is the message trigger
    const appConfig = getAppConfig()
    ...
    dsSelectorSource = {
      ...
      useDataSource: configUseDataSource,
      useDataSources: configUseDataSources || Immutable([]),
...
type FilterFlashActionSettingProps = ActionSettingProps<FilterMessageActionIMConfig> | ActionSettingProps<FlashMessageActionIMConfig>
 ...
 * @returns
 */
export function getInitConfigForFilterAndFlashMessageActions (props: FilterFlashActionSettingProps): ResultOfGetInitConfigForFilterAndFlashMessageActions {
  const messageUseDataSource: IMUseDataSource = getInitMessageUseDataSourceForFilterAndFlashMessageActions(props)
  const actionUseDataSource: IMUseDataSource = getInitActionUseDataSourceForFilterAndFlashMessageActions(props)
...
function getInitMessageUseDataSourceForFilterAndFlashMessageActions (props: FilterFlashActionSettingProps): IMUseDataSource {
  let messageUseDataSource: IMUseDataSource = null

  if (props.dataSourceId) {
    // case1: background data source is the message trigger
    if (props.config.messageUseDataSource) {
      messageUseDataSource = props.config.messageUseDataSource
    } else {
      const bgUseDataSource = getUseDataSourceByDataSourceId(props.dataSourceId)

      if (bgUseDataSource) {
        messageUseDataSource = Immutable(bgUseDataSource)
      }
    }
...
function getInitActionUseDataSourceForFilterAndFlashMessageActions (props: FilterFlashActionSettingProps): IMUseDataSource {
...
export function getDataSourceSelectorSourceDataForFilterAndFlashMessageActions (widgetId: string, triggerBgDataSourceId: string, messageType: MessageType, useDataSource: Immutable.ImmutableObject<UseDataSource>) {
  let dsSelectorSource: DataSourceSelectorSourceDataForFilterAndFlash

  if (triggerBgDataSourceId) {
    // case1: background data source is the message trigger
    const bgUseDataSource = getUseDataSourceByDataSourceId(triggerBgDataSourceId)
    const isReadOnly = true
    const useDataSources = bgUseDataSource ? Immutable([bgUseDataSource]) : Immutable([] as UseDataSource[])
    const fromRootDsIds = undefined
    const fromDsIds = Immutable([triggerBgDataSourceId])
    dsSelectorSource = {
      isReadOnly,
      useDataSources,
      fromRootDsIds,
...
function getUseDataSourceByDataSourceId (dataSourceId: string): UseDataSource {
  ...
  const ds = dataSourceId && DataSourceManager.getInstance().getDataSource(dataSourceId)

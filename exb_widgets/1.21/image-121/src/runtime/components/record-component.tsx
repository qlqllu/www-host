...
class _RecordComponent extends React.PureComponent<Props & ExtraProps, State> {
  ...
  private previousDsId: string
  private previousRecordId: string | number
  ...
  updateRecord = () => {
    ...
    this.timer = window.setTimeout(() => {
      ...
      const dsId = record?.dataSource.id
      const recordId = record?.getId()
      if (this.props.onRecordChange && (this.previousDsId !== dsId || this.previousRecordId !== recordId)) {
        this.props.onRecordChange(record)
      }
      this.previousDsId = dsId
  ...
  getSingleRecord = () => {
    if (this.props.isSelectedFromRepeatedDataSourceContext) {
      ...
      // record.fake === true means the record is mock data by list widget

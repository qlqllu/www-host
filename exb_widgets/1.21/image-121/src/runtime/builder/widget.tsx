import { React, getAppStore, appActions, Immutable, ReactDOM, appConfigUtils } from 'jimu-core'
import type { IMConfig } from '../../config'
import { type ImageResourceItemInfo, getAppConfigAction } from 'jimu-for-builder'
import { CropType, type CropParam, type CropPosition } from 'jimu-ui'
...
export default class _BuilderWidget extends React.PureComponent<OwnProps, OwnStates> {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      imageHeight: null,
      clientRect: {
        width: this.props.widgetWidth || 0,
        height: this.props.widgetHeight || 0,
        x: 0,
        y: 0,
        bottom: 0,
        left: 0,
  ...
  componentDidMount () {
    ...
    const imageUrl = appConfigUtils.processResourceUrl(this.props.config.functionConfig.imageParam.originalUrl)
    this.preloadImage(imageUrl)
    const widgetRenderer = document.querySelector(`.widget-renderer[data-widgetid='${this.props.widgetId}']`)
    if (widgetRenderer) {
      this.setState({
        clientRect: widgetRenderer.getBoundingClientRect()
      })
    }
  }
  ...
  preloadImage = (url: string) => {
    ...
    this.imgObject.onload = (e) => {
      ...
      this.setState({
        imageWidth: this.imgObject.width,
        imageHeight: this.imgObject.height
      })
    }
  }
  ...
  getCropZoomInCropTool = (cropParam, widgetWidth, widgetHeight, imageWidth: number, imageHeight: number): number => {
    ...
    let relativeZoomInWidget

    let imageWidthInSvg
    let imageHeightInSvg
    ...
    const zoomInCropTool = imageWidthInSvg * relativeZoomInWidget / imageWidth
    return zoomInCropTool
  }

  getCropPositionInCropTool = (cropParam: CropParam, zoomInCropTool: number): CropPosition => {
  ...
  render () {
    ...
    if (this.state.imageWidth && this.state.imageHeight) {
      ...
      const cropPositionInCropTool = this.getCropPositionInCropTool(cropParam, zoomInCropTool)
      const imageUrl = appConfigUtils.processResourceUrl(this.props.config.functionConfig.imageParam.originalUrl)

      return (
        <ImageCrop
          crop={cropPositionInCropTool} cropZoom={zoomInCropTool}
          imageFormat={this.props.config.functionConfig.imageParam.fileFormat} onCancelCrop={this.onCancelCrop} widgetArea={this.state.clientRect}
          cropParam={cropParam} originId={this.props.config.functionConfig.imageParam.originalId}
          widgetId={this.props.widgetId} image={imageUrl} onConfirmCrop={this.onConfirmCrop}

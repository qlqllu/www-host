import {
  ...
  type IMState, type BrowserSizeMode, classNames, type ImmutableObject
} from 'jimu-core'
...
import AttachmentWidget from './components/attachment-widget'
...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & ExtraProps, State> {
  ...
  render () {
    const {
      id, useDataSourcesEnabled: isDataSourceUsed, useDataSources, config, autoHeight, autoWidth,
      isInlineEditing, repeatedDataSource, builderSupportModules, queryObject
    } = this.props
    const { functionConfig = {} as ImmutableObject<FunctionConfig> } = config
    const {
      dynamicUrlType, imageFillMode, imageDisplayQualityMode, symbolScale, linkParam, imgSourceType, imageViewer,

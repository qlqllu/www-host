...
import { LIST_CARD_MIN_SIZE, SCROLL_BAR_WIDTH } from '../../config'
...
export interface ListRuntimeState {
  ...
  selectedRecordsId: Array<string | number>
...
const initialState: ListRuntimeState = {
  ...
  scrollBarSize: SCROLL_BAR_WIDTH,
  LayoutEntry: null
}
...
const reducer = (state: ListRuntimeState, action) => {
  switch (action.type) {
    ...
    case 'SET_DYNAMIC_ROW_HEIGHT':
      return { ...state, dynamicRowHeight: action.value }
    ...
    case 'SET_SELECTED_RECORDS':
      return { ...state, selectedRecords: action.value }
    case 'SET_SCROLL_BAR_SIZE':
      return { ...state, scrollBarSize: action.value }
    case 'SET_LIST_RUNTIME_STATE':
      return { ...state, ...action.value}
    case 'SET_LAYOUT_ENTRY':
      return { ...state, LayoutEntry: action.value}
    default:
      return state
  }
}

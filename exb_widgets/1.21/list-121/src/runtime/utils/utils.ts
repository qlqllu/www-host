import { Status, PageStyle, DS_TOOL_H, BOTTOM_TOOL_H, SelectionModeType, ListLayoutType, ItemStyle, DYNAMIC_ROW_MIN_HEIGHT, LIST_CARD_MIN_SIZE, SCROLL_BAR_WIDTH } from '../../config'
...
import type { UseDataSource, DataRecord, IMThemeVariables, LayoutItemConstructorProps, BrowserSizeMode, DataSource, RepeatedDataSource, BoundingBox, QueriableDataSource, IMState, AllWidgetProps, IMDynamicStyle } from 'jimu-core'
...
export function getCardSizeNumberInConfig (config: IMConfig, builderStatus: Status, browserSizeMode: BrowserSizeMode, widgetRect: ElementSize, scrollBarSize = SCROLL_BAR_WIDTH): ElementSize {
  ...
  let width = initCardSize(jimuUtils.toLinearUnit(cardSizeInConfig.width), widgetRect.width + config.horizontalSpace - scrollBarSize)
...
export function initCardSize (sizeUnit: LinearUnit, widgetSize: number): number {
  if (sizeUnit.unit === DistanceUnits.PERCENTAGE) {
    return Math.floor((sizeUnit.distance / 100) * widgetSize)
  } else {
...
export function getPageSize (
  ...
  const { widgetRect, columnCount, config, isHeightAuto, isWidthAuto, builderStatus, browserSizeMode, scrollBarSize } = option
  const shouldUseDynamicRowHeight = getShouldUseDynamicRowHeight(config?.layoutType, config?.autoHeight, config?.itemStyle)
  let { listHeight } = option
  const cardSize = getCardSizeNumberInConfig(config, builderStatus, browserSizeMode, widgetRect, scrollBarSize)
  let pageSize
  if (config.pageStyle === PageStyle.Scroll) {
    ...
    switch (config?.layoutType) {
      case ListLayoutType.Row:
        ...
        {
          // In dynamic row height mode, each list item height is unknown before render,
          // so use DYNAMIC_ROW_MIN_HEIGHT as the baseline to estimate page size.
          const rowHeight = shouldUseDynamicRowHeight ? DYNAMIC_ROW_MIN_HEIGHT : cardSize.height
          pageSize = Math.ceil((listHeight + config.space) / (rowHeight + config.space)) + 1
        }
        break
...
export function recordToJSON (record: DataRecord): any {
  if (!record || !record?.getData || (record as any).fake) return record
  let attributes = record.getData()
  if (attributes) {
    attributes = { ...attributes }
  }
  let geometry = record.getGeometry()
  if (geometry) {
    geometry = {... geometry}
  }
  return {
...
export function getShouldUseDynamicRowHeight (
  layoutType: ListLayoutType,
  autoHeight: boolean,
  itemStyle: ItemStyle
): boolean {
  return layoutType === ListLayoutType.Row && !!autoHeight &&
    (itemStyle === ItemStyle.Style10 || itemStyle === ItemStyle.Style11 || itemStyle === ItemStyle.Style12)
}
...
export function initBackgroundStyle (cardBackgroundStyle: IMCardBackgroundStyle, dynamicStyleOfCard?: IMDynamicStyle, theme?: IMThemeVariables) {
  if (dynamicStyleOfCard?.background) {
    ...
    if (dynamicStyleOfCard?.background?.contentColor) {
      cardBackgroundStyle = cardBackgroundStyle.set('color', dynamicStyleOfCard?.background?.contentColor)
    }
  }

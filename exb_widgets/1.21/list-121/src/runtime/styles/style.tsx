  ...
  return css`
    &.list-container {
      ...
      .bottom-boundary {
        ...
        bottom: ${layoutType === ListLayoutType.Column ? '100%' : '0'};
        right: ${layoutType === ListLayoutType.Column ? '0' : `${scrollBarSize + 40}px`};
        opacity: 0;
      }
      ${
      ...
      .widget-list-list {
        ...
        &>div:has(.preview-dynamic-style-remind) {
          z-index: 11;
        }
...
function getListMaxStyle (pageStyle: PageStyle, isHorizon: boolean, maxSize: MaxSize, topToolsH: number, bottomToolH: number) {
  if (pageStyle === PageStyle.MultiPage) {
    ...
    return isHorizon ? `max-width: ${maxSize.maxWidth} !important;` : `max-height: calc(${maxSize.maxHeight} - ${topToolsH}px - ${bottomToolH}px) !important;`
  }
}

...
export interface ExtraProps {
  ...
  setDynamicRowHeight?: (height: number) => void
}
...
> {
  ...
  lastResizeCall = null
  lastObservedHeight: number | null = null
  ...
  componentDidUpdate (preProps) {
    ...
    this.updateDynamicRowHeightObserver()
    this.updateLayoutEntryWhenLayoutIdChange(preProps)
  }

  componentDidMount () {
    const { itemIdex, toggleCardTool, config, shouldUseDynamicRowHeight } = this.props
    const {layoutType, keepAspectRatio, lockItemRatio } = config
    if (
      itemIdex === 0 &&
      ...
      this.interactable = interact(this.resizeRef.current).resizable({
        ...
        edges: {
          ...
          bottom: ((layoutType === ListLayoutType.Row && !lockItemRatio && !shouldUseDynamicRowHeight) || (layoutType === ListLayoutType.GRID && !keepAspectRatio)),
  ...
  updateDynamicRowHeightObserver = () => {
    const { itemIdex, shouldUseDynamicRowHeight, setDynamicRowHeight } = this.props
    const shouldObserve = itemIdex === 0 && window.jimuConfig.isInBuilder && shouldUseDynamicRowHeight && this.resizeRef.current
    ...
    const element = this.resizeRef.current
    this.resizeObserver = new ResizeObserver((entries) => {
      const entry = entries?.[0]
      if (!entry) return

      const height = entry.contentRect?.height
      if (!height || height === this.lastObservedHeight) return
    ...
    this.lastObservedHeight = rect.height
    setDynamicRowHeight?.(rect.height)
  }
  /**
  ...
  */
  updateLayoutEntryWhenLayoutIdChange = (preProps: ListCardProps & ExtraProps) => {
  ...
  _renderAction = () => {
    const handlers = []
    const { theme, config, shouldUseDynamicRowHeight } = this.props
    ...
    switch (layoutType) {
      ...
      case ListLayoutType.Row:
        if (shouldUseDynamicRowHeight) break
        handlers.push(
          <span
            key='9'

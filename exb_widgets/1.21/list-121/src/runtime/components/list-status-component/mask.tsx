...
import { getBottomToolH, getShouldUseDynamicRowHeight } from '../../utils/utils'
...
const MaskOfList = (props: Props) => {
  ...
  const { records, currentCardSize, dynamicRowHeight, widgetRect, showLoading, scrollBarSize } = useListRuntimeState()
  ...
  useEffect(() => {
    const listLeftPadding = getListContentLeftPadding(config, widgetRect, currentCardSize, scrollBarSize)
    setListLeftPadding(listLeftPadding)
  }, [config, widgetRect, currentCardSize, scrollBarSize])
  ...
  const firstRowHeight = React.useMemo(() => {
    return shouldUseDynamicRowHeight && dynamicRowHeight > 0 ? dynamicRowHeight : currentCardSize.height
  }, [shouldUseDynamicRowHeight, dynamicRowHeight, currentCardSize.height])

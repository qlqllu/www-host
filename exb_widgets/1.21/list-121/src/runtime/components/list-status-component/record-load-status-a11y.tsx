...
const RecordLoadStatusA11y = (props: Props) => {
  ...
  useEffect(() => {
    ...
    if (showRecordCount) {
      ...
      const statusString = nls('resultsUpdatedRemind', { total: dataTotal, selection: selected })
      newRecordLoadStatusString = queryStatus === DataSourceStatus.Loaded ? statusString : ''
    } else {
      const updatedString = records?.length > 0 ? nls('resultsUpdated') : (noDataMessage || nls('listNoData'))

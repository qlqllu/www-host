...
const ListWidget = (props: ListProps): React.ReactElement => {
  ...
  const { id, config, appMode, selectionIsSelf, selectionIsInSelf, useDataSources, layouts, builderStatus, builderSupportModules, parentSize, browserSizeMode, dispatch, selectionStatus, layoutId, layoutItemId, isHeightAuto, isWidthAuto, } = props
  const { dataSource, showSelectionOnly, widgetRect, currentCardSize, firstItemIndexInViewport, records, scrollBarSize } = useListRuntimeState()
  ...
  useEffect(() => {
    ...
    const currentCardSize = getCardSizeNumberInConfig(config, builderStatus, browserSizeMode, widgetRect, scrollBarSize)
  ...
  const notAutoResizeCardSize = React.useMemo(() => {
    if (!isWidthAuto || config.layoutType !== ListLayoutType.GRID) {
      return false
    } else {
      const cardSize = config.cardConfigs?.[builderStatus]?.cardSize?.[browserSizeMode]
      if (`${cardSize?.width}`?.includes('%')) {
        return true
      } else {
        return false
      }
    }
  }, [isWidthAuto, config.cardConfigs, config.layoutType, builderStatus, browserSizeMode])
  ...
  const getPageSize = React.useCallback((options) => {
    ...
    const columnCount = config?.layoutType === ListLayoutType.GRID ? getItemColumnCount(config, widgetRect, currentCardSize, scrollBarSize) : null
  ...
  const listProps = React.useMemo(() => {
    ...
    return listProps as ListProps
  }, [id, config, selectionIsInSelf, selectionIsSelf, builderStatus, layouts, builderSupportModules])
  ...
  useEffect(() => {
    ...
    //When the accordion widget is collapsed, its height is 0.
    //At this time, resetting the page size will cause the scrollbar position to change.
    //https://devtopia.esri.com/Beijing-R-D-Center/ExperienceBuilder-Web-Extensions/issues/26345
    ...
    <Fragment>
      ...
      >
        <ListElement
          ...
          notAutoResizeCardSize={notAutoResizeCardSize}
          listProps={listProps}
        />
        <TransparentMask
          config={config}

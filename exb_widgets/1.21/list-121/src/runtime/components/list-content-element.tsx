...
import type { ListDivSize, ElementSize, WidgetRect, ListProps, ItemsRenderedOptions } from '../../config'
...
import { getListMaxSize, checkIsLastPage, getBottomToolH, getListHeight } from '../utils/utils'
...
export class Widget extends React.PureComponent<ListProps & Props, ListStates> {
  ...
  componentDidMount () {
    let LayoutEntryInState
    if (window.jimuConfig.isInBuilder) {
      LayoutEntryInState = this.props.builderSupportModules.LayoutEntry
    } else {
      LayoutEntryInState = LayoutEntry
    }
    this.context.dispatch({type: 'SET_LAYOUT_ENTRY', value: LayoutEntryInState})
    this.setState({
      isMount: true
    })
  }
  ...
  layoutPropertiesChange = (preProps, preState): boolean => {
    ...
    // listen layout properties change and then update list
    let newCardSize = this.getCardSizeNumberInConfig()
    ...
    const isEqualCardSizeByListLayout = !listUtils.isEqualCardSizeByListLayout(oldCardSize, newCardSize, config.layoutType) // for change template

    if (isListLayoutChange || isEqualCardSizeByListLayout || isWidgetPositionChange) {
      needUpdateCardToolPosition = true
      if (isEqualCardSizeByListLayout || isListLayoutChange) {
        refreshList = true
        newCardSize = this.initCardSizeInCardConfig(newCardSize)
  ...
  initCardSizeInCardConfig = (cardSize: ElementSize) => {
    const { config } = this.props
    if (config?.layoutType === ListLayoutType.Row) {
      cardSize.width = this.context.state.widgetRect?.width
    } else if(config?.layoutType === ListLayoutType.Column) {
      cardSize.height = this.getListElementHeight()
    }
    return cardSize
  }

  getListElementHeight = () => {
    const { showBottomTools, showTopTools, paginatorDiv } = this.props
    const bottomToolH = getBottomToolH(paginatorDiv, showBottomTools)
    return getListHeight(this.context.state.widgetRect, bottomToolH, showTopTools)
  }
  ...
  getCardSizeNumberInConfig = (widgetRect?: WidgetRect, props?: ListProps) => {
    ...
    const { scrollBarSize } = this.context.state
    return listUtils.getCardSizeNumberInConfig(config, builderStatus, browserSizeMode, widgetRect, scrollBarSize)
  }
  ...
  scrollToIndex = (index: number, type: string = 'start') => {
    const { config } = this.props
    if (!this.context.state?.records || this.context.state.records?.length === 0) {
      return
    }
  ...
  lastScrollOffset = 0
  handleListScroll = (uIEventHandler) => {
    ...
    this.lastScrollOffset = config?.layoutType === ListLayoutType.Column ? Math.abs(uIEventHandler?.currentTarget?.scrollLeft) : Math.abs(uIEventHandler?.currentTarget?.scrollTop)
  ...
  initNewCardSize = (newCardSize: ElementSize) => {
    ...
    const { widgetRect, scrollBarSize } = this.context.state
    return initNewCardSize(newCardSize, config, builderStatus, browserSizeMode, widgetRect, scrollBarSize)
  }
  ...
  getDsSelectRecords = () => {
    return this.context.state.selectedRecords
  }

  getListStyle = () => {
    const { config, appMode, theme, isHeightAuto, isWidthAuto, showBottomTools, showTopTools, paginatorDiv } = this.props
    const { currentCardSize, widgetRect, scrollBarSize }= this.context.state
    ...
    const listStyleOption = {
      ...
      listLeftPadding: getListContentLeftPadding(config, widgetRect, currentCardSize, scrollBarSize),
  ...
  onItemsRendered = (option: ItemsRenderedOptions) => {
    const { config } = this.props
    // All index params are numbers.
    if (config?.layoutType === ListLayoutType.Row) {
      const { visibleRows } = option.row
      this.listVisibleStartIndex = visibleRows?.startIndex
      this.listVisibleStopIndex = visibleRows?.stopIndex
    } else {
      const { visibleCells } = option.grid
      const { columnStartIndex, columnStopIndex, rowStartIndex, rowStopIndex } = visibleCells
      this.listVisibleStartIndex = this.getItemIndexByRowAndColumnIndex(rowStartIndex, columnStartIndex)
      this.listVisibleStopIndex = this.getItemIndexByRowAndColumnIndex(rowStopIndex, columnStopIndex)
    }
  ...
  getItemColumnCount = () => {
    const { config } = this.props
    const { currentCardSize, widgetRect, scrollBarSize } = this.context.state
    return getItemColumnCount(config, widgetRect, currentCardSize, scrollBarSize)
  }
  ...
  renderContentOfListWidget = () => {
    ...
    const { config, useDataSources, id, selectionIsInSelf, selectionIsSelf, appMode, listProps, isEditing, showBottomTools, showTopTools, paginatorDiv, setPaginatorDivRef} = this.props
    const { dataSource, page, records, pageSize, selectedRecords, LayoutEntry } = this.context.state
  ...
  render () {
    ...
    const { createDataSourceFailed, isMount } = this.state
    const { LayoutEntry } = this.context.state
    return (
      <React.Fragment>
        ...
        <ListResizeDetector
          ...
          notAutoResizeCardSize={this.props.notAutoResizeCardSize}
          id={id}
          showTopTools={showTopTools}
          showBottomTools={showBottomTools}
          paginatorDiv={paginatorDiv}

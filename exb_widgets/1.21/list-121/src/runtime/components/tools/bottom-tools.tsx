  ...
  return (
    <div className='bottom-tools w-100 d-flex align-items-center justify-content-center pl-2 pr-2' css={STYLE}>
      ...
      {config.pageStyle !== PageStyle.MultiPage && <div className='d-flex scroll-navigator'>
        ...
        >
          {config.layoutType === ListLayoutType.Column ? <LeftOutlined autoFlip size={12}/> : <UpOutlined autoFlip size={12}/>}
        </Button>
        <Button
          title={nls('next')}
        ...
        >
          {config.layoutType === ListLayoutType.Column ? <RightOutlined autoFlip/> : <DownOutlined autoFlip/>}
        </Button>
      </div>}
    </div>
  )

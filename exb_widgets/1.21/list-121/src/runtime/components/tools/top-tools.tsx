/** @jsx jsx */
import { jsx, React, hooks, dataSourceUtils, DataSourceStatus, classNames, getAppStore, ReactRedux, LayoutItemType, MessageManager, DataSourceFilterChangeMessage, defaultMessages as jimuCoreDefaultMessage, AppMode } from 'jimu-core'
import type { DataSource, DataRecord, UseDataSource, IMState, FeatureLayerDataSource, IMSqlExpression, ImmutableArray, DataRecordSet } from 'jimu-core'
...
const { useClearDataSourceFilters } = dataSourceUtils
...
const TopTools = (props: Props) => {
  ...
  const applyFilterStatusRef = useRef({
    applySearchFilter: false,
    applyOtherFilter: false
  })
  ...
  const { resetToken } = useClearDataSourceFilters(id, useDataSources)
  ...
  }, [listRuntimeDispatch, selectRecordsAndPublishMessageAction])
  ...
  const getAllFieldsOfListAndChildrenWidget = React.useCallback((): string[] => {
  ...
  const toggleSearchBoxVisible = React.useCallback((isVisible = false) => {
  ...
  }, [isShowDataAction, listWidth])

  const getSearchSuggestions = React.useCallback((searchText) => {
  ...
  const publishDataFilterChangeMessage = React.useCallback(() => {
    ...
    MessageManager.getInstance().publishMessage(new DataSourceFilterChangeMessage(id, [dataSource.id], clearFilterDataSourceIds))
  }, [id, dataSource])

  const handleSearchSubmit = React.useCallback((newSearchText: string, isEnter = false) => {
    ...
    applyFilterStatusRef.current = {
      ...applyFilterStatusRef.current,
      applySearchFilter: !!newSearchText
    }
    publishDataFilterChangeMessage()
  }, [searchText, listRuntimeDispatch, publishDataFilterChangeMessage])

  const handleSearchTextChange = React.useCallback((searchText) => {
  ...
  }, [getSearchSuggestions, handleSearchSubmit])

  const getClassNameListToolsWithoutSearchTools = React.useCallback((): string => {
  ...
  const handleFilterChange = React.useCallback((sqlExprObj: IMSqlExpression) => {
    listRuntimeDispatch({type: 'SET_CURRENT_FILTRE', value: sqlExprObj})
    listRuntimeDispatch({ type: 'SET_PAGE', value: 1 })
    applyFilterStatusRef.current = {
      ...applyFilterStatusRef.current,
      applyOtherFilter: !!sqlExprObj
    }
    publishDataFilterChangeMessage()
  }, [listRuntimeDispatch, publishDataFilterChangeMessage])

  const handleFilterApplyChange = React.useCallback((applied: boolean) => {
    listRuntimeDispatch({type: 'SET_FILTER_APPLIED', value: applied})
    applyFilterStatusRef.current = {
      ...applyFilterStatusRef.current,
      applyOtherFilter: applied
    }
    publishDataFilterChangeMessage()
  }, [listRuntimeDispatch, publishDataFilterChangeMessage])

  const checkIsClearFilter = (): boolean => {
    const { applySearchFilter, applyOtherFilter } = applyFilterStatusRef.current
    return !applySearchFilter && !applyOtherFilter
  }

  const renderListToolsWithoutSearchTools = React.useCallback((ds: DataSource, queryStatus?: DataSourceStatus) => {
  ...
  }, [getClassNameListToolsWithoutSearchTools, config, sortOptionName, handleSortOptionChange, id, showSortString, currentFilter, appMode, filterApplied, nls, handleFilterChange, handleFilterApplyChange, theme, showSelectionOnly, handleShowSelectionClick, handleClearSelectionClick, updateTopToolsContainer, handleRefreshList, resetToken])

  const renderSearchTools = React.useCallback((ds: DataSource, queryStatus?: DataSourceStatus) => {
  ...
  }, [isEditing, listWidth, config, nls, isSearchBoxVisible, theme, searchText, handleSearchTextChange, handleSearchSubmit, searchSuggestion, showLoading, toggleSearchBoxVisible, appMode, resetToken])

  const renderListTopToolsInPopper = React.useCallback((
    ds: DataSource,
  ...
  }, [isEditing, widgetRect?.width, isShowDataAction, isOpenTopToolsPopper, theme, renderSearchTools, isSearchBoxVisible, renderListToolsWithoutSearchTools])

  const selectedRecordsForDataAction = React.useMemo(() => {
    return dataSource?.getSelectedRecords?.() || selectRecords || []
  }, [dataSource, selectRecords])

  const fieldsForDataAction = React.useMemo(() => {
    return getAllFieldsOfListAndChildrenWidget()
  }, [getAllFieldsOfListAndChildrenWidget])

  const dataSets = React.useMemo(() => {
    return [{ dataSource: dataSource, records: selectedRecordsForDataAction, name: dataName, type: 'selected', fields: fieldsForDataAction }] as DataRecordSet[]
  }, [dataName, dataSource, fieldsForDataAction, selectedRecordsForDataAction])

  const showDataAction = React.useMemo(() => {
    return dataSource && isShowDataAction
  }, [dataSource, isShowDataAction])

  const shouldUseMarginLeftClass = React.useMemo(() => {
    return listWidth < listToolsMinSize
  }, [listToolsMinSize, listWidth])
  ...
  return (
    <div className='datasource-tools w-100' css={getTopToolStyle(showToolsWithoutDataAction, theme, (listTopRightToolsDivRef.current?.clientWidth || 0), showDataAction)}>
      <div className="d-flex align-items-center">
        ...
        {showDataAction && <div className={classNames('list-data-action position-relative', { 'm-left': shouldUseMarginLeftClass })} data-testid="data-action">
          <DataActionList
            widgetId={id}
            dataSets={dataSets}

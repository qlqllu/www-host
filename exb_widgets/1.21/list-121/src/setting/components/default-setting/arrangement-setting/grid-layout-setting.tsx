...
import { Slider, Tooltip, TextAlignValue, DistanceUnits, Button, utils as uiUtils, type LinearUnit, defaultMessages as jimuUIDefaultMessages } from 'jimu-ui'
  ...
  return (
    <div className='mt-4'>
      <SettingRow
        ...
        flow='wrap'/>
      <SettingRow label={nls('width')}>
        <div role='group' className='list-size-edit' aria-label={nls('width')} aria-describedby='sizeDescribeByMessage'>
          <SizeEditor
            disableModeSelect
            label='W'
      ...
      >
        <Tooltip title={nls('listKeepAspectRatio')} placement='bottom'>
          <Button type='tertiary' size='sm' disableHoverEffect icon aria-label={nls('listKeepAspectRatio')} className='icon' onClick={() => { handleKeepAspectRatioChange(config) }}>
            {config?.keepAspectRatio ? <LockOutlined size='s' /> : <UnlockOutlined size='s' />}
          </Button>
        </Tooltip>
      </div>
      <SettingRow role='group' label={nls('height')} aria-label={nls('height')}>
      ...
      <SettingRow role='group' label={nls('alignment')} aria-label={nls('alignment')}>
        <TextAlignment

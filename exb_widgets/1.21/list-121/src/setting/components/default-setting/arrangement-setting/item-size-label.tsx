...
const ItemSizeLabel = (props: Props) => {
  ...
  useEffect(() => {
    ...
    if (isGrid) {
      ...
      labelString = isVertical ? nls('itemHeight') : nls('itemWidth')
    }
  ...
  const getBrowserIcons = (iconSize: number, browserSizeMode: BrowserSizeMode, showCardSetting, layouts) => {
    ...
    return (
      <div className='d-flex justify-content-between align-items-center'>
        ...
        <div id='sizeDescribeByMessage' className='sr-only'>
          {isDesktop && <Fragment>{desktopLabel}</Fragment>}
          {isPad && <Fragment>{padLabel}</Fragment>}
          {isMobile && <Fragment>{mobileLabel}</Fragment>}
        </div>
      </div>
    )
  }

...
const ArrangementSetting = (props: Props) => {
  ...
  const handleLayoutChange = (layoutType: ListLayoutType) => {
    let newConfig = config
      ...
      .set('horizontalSpace', DEFAULT_SPACE)
      .set('verticalSpace', DEFAULT_SPACE)
  ...
  return (
    <SettingSection>
      ...
      >
        <SettingRow className="mt-2" flow='wrap' label={nls('layout')}>
          <div className='d-flex align-items-center w-100 list-layout-select-con' aria-label={nls('layout')} role='radiogroup'>
            <Button
              icon
              title={nls('layoutRow')}
              aria-label={nls('layoutRow')}
              active={config?.layoutType === ListLayoutType.Row}
              onClick={() => { handleLayoutChange(ListLayoutType.Row) }}
              className='flex-grow-1'
              size='lg'
              role='radio'
              aria-checked={config?.layoutType === ListLayoutType.Row}
            >
              <Icon className='style-img w-100 h-100' icon={require('../../../assets/row.png')}/>
            </Button>
            <Button
              icon
              title={nls('layoutColumn')}
              aria-label={nls('layoutColumn')}
              active={config?.layoutType === ListLayoutType.Column}
              onClick={() => { handleLayoutChange(ListLayoutType.Column) }}
              className='ml-2 flex-grow-1'
              size='lg'
              role='radio'
              aria-checked={config?.layoutType === ListLayoutType.Column}>
              <Icon className='style-img w-100 h-100' icon={require('../../../assets/column.png')}/>
            </Button>
            <Button
              icon
              title={nls('layoutGrid')}
              aria-label={nls('layoutGrid')}
              active={config?.layoutType === ListLayoutType.GRID}
              onClick={() => { handleLayoutChange(ListLayoutType.GRID) }}
              className='ml-2 flex-grow-1'
              size='lg'
              role='radio'
              aria-checked={config?.layoutType === ListLayoutType.GRID}
            >

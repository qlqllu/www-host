System.register([], function (_export) {return {execute: function () {_export({
  _widgetLabel: 'Карта',
  _action_displayFeatureSet_label: 'Отобразить набор объектов',
  _action_panTo_label: 'Переместить к',
  _action_zoomToFeature_label: 'Приблизить к',
  _action_selectFeature_label: 'Выбрать объект',
  _action_flash_label: 'Подсветить',
  _action_filter_label: 'Фильтр',
  _action_showOnMap_label: 'Показать на карте',
  _action_addToMap_label: 'Добавить к карте',
  showOnMapData: 'Показывать данные на карте',
  addedData: 'добавленные данные',
  failToAddTheDataOnMap: 'Не удалось добавить данные.',
  addToMapData: 'Добавить к данным карты'
})}}});
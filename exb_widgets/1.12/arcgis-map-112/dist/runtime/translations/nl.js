System.register([], function (_export) {return {execute: function () {_export({
  _widgetLabel: 'Kaart',
  _action_displayFeatureSet_label: 'Featureset weergeven',
  _action_panTo_label: 'Pannen naar',
  _action_zoomToFeature_label: 'Zoomen naar',
  _action_selectFeature_label: 'Object selecteren',
  _action_flash_label: 'Flash',
  _action_filter_label: 'Filteren',
  _action_showOnMap_label: 'Toon op kaart',
  _action_addToMap_label: 'Aan kaart toevoegen',
  showOnMapData: 'Weergeven op kaartgegevens',
  addedData: 'toegevoegde gegevens',
  failToAddTheDataOnMap: 'Kan de gegevens niet toevoegen.',
  addToMapData: 'Aan kaartgegevens toevoegen'
})}}});
System.register([], function (_export) {return {execute: function () {_export({
  _widgetLabel: 'Mapa',
  _action_displayFeatureSet_label: 'Zobraziť sadu prvkov',
  _action_panTo_label: 'Posun na',
  _action_zoomToFeature_label: 'Priblížiť na',
  _action_selectFeature_label: 'Vybrať prvok',
  _action_flash_label: 'Záblesk',
  _action_filter_label: 'Filter',
  _action_showOnMap_label: 'Zobraziť v mape',
  _action_addToMap_label: 'Pridať do mapy',
  showOnMapData: 'Zobraziť na dátach mapy',
  addedData: 'pridané dáta',
  failToAddTheDataOnMap: 'Nepodarilo sa pridať dáta.',
  addToMapData: 'Pridať do mapových dát'
})}}});
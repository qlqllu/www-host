System.register([], function (_export) {return {execute: function () {_export({
  _widgetLabel: 'Karte',
  _action_displayFeatureSet_label: 'Parādīt elementu kopu',
  _action_panTo_label: 'Pārbīdīt uz',
  _action_zoomToFeature_label: 'Tālummaiņa',
  _action_selectFeature_label: 'Atlasīt elementu',
  _action_flash_label: 'Flash',
  _action_filter_label: 'Filtrs',
  _action_showOnMap_label: 'Parādīt kartē',
  _action_addToMap_label: 'Pievienot kartei',
  showOnMapData: 'Rādīt ar kartes datiem',
  addedData: 'pievienotie dati',
  failToAddTheDataOnMap: 'Neizdevās pievienot datus.',
  addToMapData: 'Pievienot kartes datiem'
})}}});
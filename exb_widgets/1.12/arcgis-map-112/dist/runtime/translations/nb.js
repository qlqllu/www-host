System.register([], function (_export) {return {execute: function () {_export({
  _widgetLabel: 'Kart',
  _action_displayFeatureSet_label: 'Vis geoobjektsett',
  _action_panTo_label: 'Panorer til',
  _action_zoomToFeature_label: 'Zoom til',
  _action_selectFeature_label: 'Velg geoobjekt',
  _action_flash_label: 'Flash',
  _action_filter_label: 'Filter',
  _action_showOnMap_label: 'Vis på kart',
  _action_addToMap_label: 'Legg til kart',
  showOnMapData: 'Vis på kartdata',
  addedData: 'la til data',
  failToAddTheDataOnMap: 'Kan ikke legge til dataene.',
  addToMapData: 'Legg til kartdata'
})}}});
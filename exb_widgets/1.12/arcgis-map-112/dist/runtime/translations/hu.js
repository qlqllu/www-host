System.register([], function (_export) {return {execute: function () {_export({
  _widgetLabel: 'Térkép',
  _action_displayFeatureSet_label: 'Vektoroselem-készlet megjelenítése',
  _action_panTo_label: 'Mozgatás',
  _action_zoomToFeature_label: 'Nagyítás erre',
  _action_selectFeature_label: 'Vektoros elem kijelölése',
  _action_flash_label: 'Felvillant',
  _action_filter_label: 'Szűrő',
  _action_showOnMap_label: 'Megjelenítés a térképen',
  _action_addToMap_label: 'Hozzáadás térképhez',
  showOnMapData: 'Megjelenítés a térképadaton',
  addedData: 'adatok hozzáadva',
  failToAddTheDataOnMap: 'Az adatok hozzáadása nem sikerült.',
  addToMapData: 'Hozzáadás a térképadatokhoz'
})}}});
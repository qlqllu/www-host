System.register([], function (_export) {return {execute: function () {_export({
  _widgetLabel: 'แผนที่',
  _action_displayFeatureSet_label: 'แสดงชุดฟีเจอร์',
  _action_panTo_label: 'เลื่อนไปที่',
  _action_zoomToFeature_label: 'ขยายไปยัง',
  _action_selectFeature_label: 'เลือกฟีเจอร์',
  _action_flash_label: 'แฟลช',
  _action_filter_label: 'ตัวกรอง',
  _action_showOnMap_label: 'แสดงบนแผนที่',
  _action_addToMap_label: 'เพิ่มไปยังแผนที่',
  showOnMapData: 'แสดงบนข้อมูลแผนที่',
  addedData: 'ข้อมูลที่เพิ่มแล้ว',
  failToAddTheDataOnMap: 'ล้มเหลวในการเพิ่มข้อมูล',
  addToMapData: 'เพิ่มลงในข้อมูลแผนที่'
})}}});
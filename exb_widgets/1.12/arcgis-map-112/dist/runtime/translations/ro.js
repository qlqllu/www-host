System.register([], function (_export) {return {execute: function () {_export({
  _widgetLabel: 'Hartă',
  _action_displayFeatureSet_label: 'Afișare set de obiecte spațiale',
  _action_panTo_label: 'Panoramare la',
  _action_zoomToFeature_label: 'Transfocare la',
  _action_selectFeature_label: 'Selectare obiect spațial',
  _action_flash_label: 'Flash',
  _action_filter_label: 'Filtru',
  _action_showOnMap_label: 'Arătați pe hartă',
  _action_addToMap_label: 'Adăugare la hartă',
  showOnMapData: 'Arătați pe datele de hartă',
  addedData: 'date adăugate',
  failToAddTheDataOnMap: 'Nu s-a reușit adăugarea datelor.',
  addToMapData: 'Adăugare la datele hărții'
})}}});
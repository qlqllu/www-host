System.register([], function (_export) {return {execute: function () {_export({
  _widgetLabel: 'Peta',
  _action_displayFeatureSet_label: 'Tampilkan rangkaian fitur',
  _action_panTo_label: 'Geser hingga',
  _action_zoomToFeature_label: 'Zoom hingga',
  _action_selectFeature_label: 'Pilih fitur',
  _action_flash_label: 'Flash',
  _action_filter_label: 'Filter',
  _action_showOnMap_label: 'Tampilkan pada peta',
  _action_addToMap_label: 'Tambahkan ke peta',
  showOnMapData: 'Tampilkan di data peta',
  addedData: 'Data yang ditambahkan',
  failToAddTheDataOnMap: 'Gagal menambahkan data.',
  addToMapData: 'Tambahkan ke data peta'
})}}});